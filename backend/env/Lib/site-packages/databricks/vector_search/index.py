...
class VectorSearchIndex:
    ...
    def _get_token_for_request(self, write=False, control_plane=False, index_url=None):
        ...
        # Detect control-plane requests via URL matching, but only when the
        # caller has not already forced control_plane=True (e.g. reranker path).
    ...
    def similarity_search(
        ...
        *,
        ...
        while response["next_page_token"]:
            response = self.__get_next_page(
                query_url, response["next_page_token"], control_plane=is_reranker
            )
    ...
    def __get_next_page(self, index_url, page_token, control_plane=False):
        """
        ...
        return RequestUtils.issue_request(
            url=url,
            token=self._get_token_for_request(control_plane=control_plane),
            method="GET",
            json=json_data,
        )
